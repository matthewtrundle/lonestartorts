...
import { Lock, ShieldCheck, Truck, ArrowLeft, Tag, Check, X, Minus, Plus, Trash2, ChevronDown } from 'lucide-react';
...
export default function CheckoutPage() {
  ...
  return (
    <main className="min-h-screen bg-cream-50 pt-20 pb-8">
        <div className="container mx-auto px-4 max-w-6xl">
          ...
          <div className="mb-4 flex items-center justify-between">
            <div className="flex items-center gap-4">
              <Link
                href="/shop"
                className="inline-flex items-center gap-1 text-sm text-charcoal-600 hover:text-sunset-600 transition-colors"
              >
                <ArrowLeft className="w-4 h-4" />
                Back
              </Link>
              <h1 className="text-2xl font-display font-bold text-charcoal-950">
                {t('checkout.title')} <span className="text-lg font-normal text-charcoal-600">({itemCount} {itemCount === 1 ? 'item' : 'items'})</span>
              </h1>
            </div>
          </div>
          ...
          <div className="grid lg:grid-cols-3 gap-6">
            ...
            <div className="lg:col-span-2">
              <div className="bg-white rounded-xl shadow-soft p-4">
                <div className="space-y-3">
                  {items.map((item) => (
                    <div key={item.sku} className="flex gap-3 pb-3 border-b border-cream-200 last:border-0 last:pb-0">
                      {/* Item Image */}
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex justify-between items-start gap-2">
                          <h3 className="font-semibold text-charcoal-950 text-sm leading-tight">
                            {item.name}
                          </h3>
                          <p className="font-bold text-base text-charcoal-950 flex-shrink-0">
                            {formatPrice(item.price * item.quantity)}
                          </p>
                        </div>
                        <p className="text-xs text-charcoal-600 mb-1">{formatPrice(item.price)} each</p>
                        {/* Quantity Controls - Inline */}
                        <div className="flex items-center gap-2">
                          <div className="flex items-center bg-gray-100 rounded p-0.5">
                            <button
                              onClick={() => updateQuantity(item.sku, item.quantity - 1)}
                              className="w-6 h-6 flex items-center justify-center rounded bg-white hover:bg-gray-50 border border-gray-200"
            ...
            {/* Order Totals & Checkout - Compact */}
            <div className="lg:col-span-1">
              <div className="bg-white rounded-xl shadow-soft p-4 sticky top-20 space-y-3">
                ...
                <div className="flex justify-between items-center">
                  <span className="font-bold text-lg text-charcoal-950">Total</span>
                  <span className="font-bold text-xl text-sunset-600">
                ...
                <div className="border border-gray-200 rounded-lg overflow-hidden">
                  ...
                  >
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-xs font-medium">
                        {discountApplied ? 'Discount Applied' : 'Have a code?'}
                      </span>
                      {discountApplied && <Check className="w-3 h-3 text-green-600" />}
                    </div>
                  ...
                  {discountOpen && (
                    <div className="px-2 pb-2 border-t border-gray-100">
                      {discountApplied ? (
                        <div className="flex items-center justify-between p-2 mt-2 bg-green-50 rounded text-xs">
                        ...
                        <div className="space-y-2 mt-2">
                          <input
                            ...
                            className="w-full px-2 py-1.5 border border-gray-200 rounded text-xs focus:outline-none focus:ring-1 focus:ring-sunset-500"
                          />
                          <div className="flex gap-1">
                            <input
                              ...
                              className="flex-1 px-2 py-1.5 border border-gray-200 rounded text-xs uppercase focus:outline-none focus:ring-1 focus:ring-sunset-500"
                            />
                            <button
                              onClick={handleApplyDiscount}
                              disabled={isValidatingCode}
                              className="px-3 py-1.5 bg-gray-900 text-white text-xs rounded hover:bg-gray-800 disabled:bg-gray-400"
                            >
                ...
                <div className="flex items-center justify-center gap-3 text-[10px] text-gray-500 pt-2">
                  ...
                  <span>•</span>
                  <div className="flex items-center gap-1">
                    <Truck className="w-3 h-3 text-sunset-600" />
                    <span>Fast shipping</span>
                  </div>
                  <span>•</span>
                  <span>Not affiliated with H-E-B®</span>
                </div>
              </div>
            </div>
          </div>
