...
import { Tag, Gift, Mail, Percent, Truck, CheckCircle, XCircle, Clock, Package, Plus, Pencil, Layers } from 'lucide-react';
...
type Tab = 'overview' | 'admin' | 'static' | 'spin' | 'feedback' | 'drip';
...
export default function DiscountsPage() {
  ...
  useEffect(() => {
    ...
    if (searchParams.get('created')) {
      setNotification('Discount code created successfully!');
      setActiveTab('admin');
    } else if (searchParams.get('updated')) {
      setNotification('Discount code updated successfully!');
      setActiveTab('admin');
    } else if (searchParams.get('deleted')) {
      setNotification('Discount code deleted successfully!');
      setActiveTab('admin');
    }
  }, [searchParams]);
  ...
  const tabs: { id: Tab; label: string; count?: number }[] = [
    ...
    { id: 'admin', label: 'Admin Codes', count: data.adminCodes?.length || 0 },
    { id: 'static', label: 'Legacy Codes', count: data.staticCodes.length },
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-charcoal-600">Manage discount codes and view usage statistics</p>
        </div>
        <Link
          href="/admin/discounts/create"
          className="inline-flex items-center gap-2 px-4 py-2 text-sm font-medium text-white bg-sunset-500 rounded-lg hover:bg-sunset-600"
        >
      ...
      <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
        <div className="bg-white rounded-lg shadow p-6">
          <div className="flex items-center gap-3">
            <div className="w-12 h-12 bg-sunset-100 rounded-lg flex items-center justify-center">
              <Tag className="w-6 h-6 text-sunset-600" />
            </div>
            <div>
              <p className="text-sm text-charcoal-500">Admin Codes</p>
              <p className="text-2xl font-bold text-charcoal-900">{data.summary.totalAdminCodes || 0}</p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-lg shadow p-6">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-sm text-charcoal-500">Legacy Codes</p>
        ...
        <div className="p-6">
          ...
          {activeTab === 'admin' && (
            <div className="space-y-4">
              {data.adminCodes && data.adminCodes.length > 0 ? (
                  ...
                  <table className="min-w-full divide-y divide-charcoal-200">
                    <thead>
                      <tr>
                        ...
                        <th className="px-4 py-3 text-left text-xs font-medium text-charcoal-500 uppercase">Usage</th>
                    ...
                    <tbody className="divide-y divide-charcoal-200">
                      {data.adminCodes.map((code) => (
                        <tr key={code.id} className={!code.isActive ? 'bg-charcoal-50' : ''}>
                          ...
                          <td className="px-4 py-3">
                            <span className="inline-flex items-center gap-1 text-sm text-charcoal-700">
                              {code.type === 'percentage' && <Percent className="w-4 h-4" />}
                              {code.type === 'fixed' && <Tag className="w-4 h-4" />}
                              {code.type === 'free_shipping' && <Truck className="w-4 h-4" />}
                              {code.type === 'bogo' && <Gift className="w-4 h-4" />}
                              {code.type === 'tiered' && <Layers className="w-4 h-4" />}
                              {code.type}
                            </span>
                          </td>
                          <td className="px-4 py-3 text-sm text-charcoal-600 max-w-xs truncate">
                            {code.description || code.name}
                          </td>
                          <td className="px-4 py-3 text-sm text-charcoal-600">
                            {code.currentUsageCount}
                            {code.maxUsageTotal ? ` / ${code.maxUsageTotal}` : ''}
                          </td>
                          <td className="px-4 py-3">
                            {!code.isActive ? (
                              <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded text-xs font-medium bg-charcoal-100 text-charcoal-600">
                          ...
                          <td className="px-4 py-3">
                            <Link
                              ...
                              className="inline-flex items-center gap-1 px-2 py-1 text-sm text-sunset-600 hover:bg-sunset-50 rounded"
                            >
                ...
                <div className="text-center py-12 bg-charcoal-50 rounded-lg">
                  <Tag className="w-12 h-12 text-charcoal-300 mx-auto mb-4" />
                  <p className="text-charcoal-600 mb-4">No admin discount codes yet</p>
                  <Link
                    href="/admin/discounts/create"
                    className="inline-flex items-center gap-2 px-4 py-2 text-sm font-medium text-white bg-sunset-500 rounded-lg hover:bg-sunset-600"
                  >
